...
class AlgoConfig(BaseConfig):
    ...
    # FEPO (Future Entropy PO): token-level shaping applied to advantages after GRPO (see ``verl.trainer.ppo.fepo``).
    # lowtail_adv keys include: alpha, beta, low_tail_neg_adv_penalty.
    # high_entropy_select_mode=threshold|top_ratio. When using top_ratio, set high_entropy_top_ratio (e.g., 0.1 for ~top 10% tokens per response).
