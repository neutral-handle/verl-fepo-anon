...
def compute_grpo_s_outcome_advantage(
    ...
    **Advantages** — same as Eq. (1) but with :math:`\\hat{r}` replacing :math:`r` (Appendix B, Eq. (46)):
    ...
    with torch.no_grad():
        ...
        # Negative-path β1·r^-: explicit override > convention (dapo -> r^-=-1; grpo -> use batch score, often 0)
        ...
        # Entropy may contain nan/inf under bf16 / vLLM proxy; handle it to avoid propagating nan into advantages.
        ...
        for gid in unique_groups:
            ...
            if n_pos > 0:
                Hp = Hbar[pos_rows]
                sum_hp = Hp.sum()
                # Avoid numerical issues when sum_hp≈0; fall back to uniform share 1/n_pos so each positive trajectory gets +beta2.
